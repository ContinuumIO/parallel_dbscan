...
namespace NWUClustering
{
    ...
    {
        // writing point id and cluster id pairs per line, noise has cluster id
        // 0
        ...
        cout << "Total points " << noise + sum_points
             << " pt_in_cls " << sum_points
             << " noise " << noise << endl;
    ...
    void ClusteringAlgo::writeClusters_uf(ostream& o)
    {
        ...
        for(i = 0; i < m_pts->m_i_num_points; i++)
        {
            ...
            // compress the tree to reduce the height of the branch of the tree
            // to 1
            j = i;
        ...
        for(i = 0; i < m_pts->m_i_num_points; i++)
        {
            ...
            {
                // get the size of cluster count: this will happen only if i is
                // a root
                count++;
...
#pragma omp barrier
            ...
            for(i = lower; i < upper; i++)
            {
                ...
                dbs.m_kdtree->r_nearest_around_point(pid, 0,
                                                     dbs.m_epsSquare, ne);
        ...
        omp_lock_t *nlocks;
        nlocks = (omp_lock_t *) malloc(dbs.m_pts->m_i_num_points *
                                       sizeof(omp_lock_t));
...
#pragma omp parallel for \
    private(i, v1, v2, root1, root2, size, tid) \
    shared(maxthreads, merge, nlocks)
        ...
        for(tid = 0; tid < maxthreads; tid++)
        {
            ...
            for(i = 0; i < size; i++)
            {
                ...
                else if(dbs.m_member[v2] == 0)
                {
                    ...
                    if(dbs.m_member[v2] == 0)
                    {
                         // if v2 is not a member yet
                        con = 1;
                        dbs.m_member[v2] = 1;
                    }
